...
<body class="min-h-screen flex flex-col">
    ...
    <nav class="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-dark-900/95 backdrop-blur-xl border-t border-dark-700 safe-area-bottom">
        <div class="flex items-center justify-around h-16 px-2">
            <a href="{{ route('home') }}" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-colors {{ request()->routeIs('home') ? 'text-primary-400' : 'text-dark-400 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                <span class="text-[10px] font-medium">Beranda</span>
            </a>
            <a href="{{ route('shop.index') }}" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-colors {{ request()->routeIs('shop.*') ? 'text-primary-400' : 'text-dark-400 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
                <span class="text-[10px] font-medium">Toko</span>
            </a>
            @auth
            <a href="{{ route('cart.index') }}" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-colors relative {{ request()->routeIs('cart.*') ? 'text-primary-400' : 'text-dark-400 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                @if(($cartCount ?? 0) > 0)
                    <span class="absolute top-1 right-1 bg-primary-500 text-white text-[9px] rounded-full w-4 h-4 flex items-center justify-center font-bold leading-none">{{ $cartCount > 9 ? '9+' : $cartCount }}</span>
                @endif
                <span class="text-[10px] font-medium">Keranjang</span>
            </a>
            <a href="{{ route('wishlist.index') }}" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-colors {{ request()->routeIs('wishlist.*') ? 'text-primary-400' : 'text-dark-400 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                <span class="text-[10px] font-medium">Wishlist</span>
            </a>
            <a href="{{ route('profile.edit') }}" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-colors {{ request()->routeIs('profile.*') ? 'text-primary-400' : 'text-dark-400 hover:text-white' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                <span class="text-[10px] font-medium">Profil</span>
            </a>
            @else
            <a href="{{ route('login') }}" class="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl text-dark-400 hover:text-white transition-colors">
    ...
    {{-- Footer --}}
    <footer class="bg-dark-950 border-t border-dark-800 mt-16 mb-16 md:mb-0">
